...
def create_report(tg_id, temp, condition, feels_like, wind_speed, pressure_mm, city):
    session = Session()
    user = session.query(User).filter(User.tg_id == tg_id).first()
    new_report = WeatherReport(temp=temp, condition=condition, feels_like=feels_like,
                               wind_speed=wind_speed, pressure_mm=pressure_mm,
